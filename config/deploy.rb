...
set :rvm_custom_path, '~/.rvm' # TODO: Switch this to '~/.rvm-alma8' after we migrate to AlmaLinux

set :remote_user, 'ldpdserv'
set :application, 'fulltext'
set :repo_url, "git@github.com:cul/ldpd-fulltext.git"
set :deploy_name, "#{fetch(:application)}_#{fetch(:stage)}"
# used to run rake db:migrate, etc
set :rails_env, fetch(:deploy_name)

# Default deploy_to directory is /var/www/my_app_name
set :deploy_to, "/opt/passenger/ldpd/#{fetch(:deploy_name)}"
